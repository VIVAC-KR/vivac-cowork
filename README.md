...
문서 성격별로 표준 하위 폴더(`decisions/`, `backlog/`, `reviews/`, `projects/`)를 씁니다. 카테고리 기준, 파일명, 톤, 문서별 템플릿은 [docs/authoring-rule.md](docs/authoring-rule.md)에 정리돼 있습니다 — 이 파일은 `docs/` 안에 있어 심볼릭 링크로 각 repo에도 공유되고, `.claude/rules/`로 한 번 더 걸어두면 문서를 쓸 때 자동으로 로드됩니다(설정 방법은 [SYMLINK-SETUP.md](SYMLINK-SETUP.md) 참고).
