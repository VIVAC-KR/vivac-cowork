...
| 구성 | 목적 → 동작 → 확정 계약 → 수용 기준 | User Story → Requirements → Success Criteria |
| 이어지는 것 | 없음 (그 자체가 정본) | `plan.md` → `tasks.md` → 구현 |
...
5. **결정이 아니면 ADR로 쓰지 않습니다.** 장애는 `docs/<약칭>/incidents/`, 미해결 항목은 `backlog/`, 현재 사실은 레퍼런스 문서에 씁니다.
...
- **재기술 금지** — 다른 문서가 정의한 것을 다시 쓰지 않고 링크합니다.
...
| 유형 | 템플릿 |
|---|---|
| Feature Specification | `.specify/templates/` (결정 2 확정 후 경로 고정) |
