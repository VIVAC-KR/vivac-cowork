...
| **기능 명세**   | `vivac-cowork/docs/features/`     | 화면별 기능 계약 — 자족적 화면 명세(§2.2)          |
...
이 금지는 **저장소 사이**에 적용됩니다. **계층 사이**에는 Constitution 원칙 V의 한정 규정이 적용되어, `docs/features/`는 정본 지목 표시 · 정본과 동일한 값 · 정본 수정 시 동반 갱신 세 조건을 모두 지키는 한 제품·데이터 정의를 문서 안에 옮겨 적을 수 있습니다(§2.1 · §2.2). 두 규칙은 충돌하지 않습니다 — 저장소별 재서술은 정본을 지목하지도, 함께 갱신되지도 않기 때문입니다.
...
| 데이터 정의(category 어휘, 노지 등재 요건, 채움률 목표) | `docs/product/PRODUCT.md` §4 (사본: `docs/features/`)   |
| 신뢰도·노출 **정책**                         | `docs/product/PRODUCT.md` §4.3 (사본: `docs/features/`) |
| 정보 구조·사이트맵·라우팅                        | `docs/product/ia.md`                       |
| 기능 후보와 우선순위                           | `docs/product/business-feature-roadmap.md` |


> "사본"은 `docs/features/`가 원칙 V 한정 규정에 따라 정본을 지목하고 옮겨 적은 같은 값입니다. 두 곳에서 같은 값을 만나더라도 **정본은 위 표의 문서**이며, 값이 어긋나면 정본이 맞고 기능 명세가 틀린 것입니다. 정본을 고칠 때는 사본을 실은 기능 명세를 함께 갱신해야 그 수정이 완료됩니다.
...
| 정보        | 정본                      |
| --------- | ----------------------- |
| 화면별 기능 계약 | `docs/features/<화면>.md` |


[DOCUMENTATION.md](DOCUMENTATION.md) §1.2의 10개 절 구성으로 기술하며, 템플릿은 `docs/meta/templates/feature-template.md`입니다. 저장소를 넘는 계약이므로 모든 구현 저장소가 이 문서를 참조합니다.

각 화면 문서는 **자족적**입니다. 구현자가 다른 문서를 열지 않고도 한 화면을 만들 수 있어야 하므로, 정본이 정의한 값을 정본 지목과 함께 옮겨 적습니다(Constitution 원칙 V 한정 규정). 옮겨 적은 값의 정본은 여전히 §2.1·§2.3의 문서입니다.

라우트가 없는 공용 로직 문서는 `docs/features/_shared/`에 둡니다. 화면 명세가 아니므로 10개 절 구성과 자족성 요구가 적용되지 않습니다.
...
`docs/features/`가 정본 지목을 붙인 사본을 실을 수 있는 것과 달리 **`specs/`는 사본을 싣지 않고 델타만 씁니다.** 수명이 짧아 사본을 정본과 맞춰 줄 동반 갱신 의무가 성립하지 않기 때문입니다([DOCUMENTATION.md](DOCUMENTATION.md) §4.1 규칙 1·5).
