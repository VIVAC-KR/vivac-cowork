...
>
> ⚠️ **구현 완료된 설계 스냅샷.** 짝 문서(backend.md)와 동일하게, 실제 구현에서
> 리소스 경로는 `/v1/admin/*`이 아닌 **`/v1/internal/*`**로 확정됐다 (로그인만
> `/v1/admin/auth/google`). 아래 코드 스니펫·리소스 등록 예시의 `/v1/admin/*`
> 경로는 옛 내용 그대로이므로 실제 연동 시 참고하지 말 것 — 현재 엔드포인트
> 목록은 [architecture.md](../architecture.md)가 기준이다.
